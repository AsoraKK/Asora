...
class ModerationService {
  ...
  ModerationService(this.baseUrl, {http.Client? httpClient}) : _http = (httpClient ?? http.Client());

  Future<Map<String, dynamic>> fetchReviewQueue({required String accessToken, int page = 1, int pageSize = 20, String status = 'pending'}) async {
    final uri = Uri.parse('$baseUrl/api/moderation/review-queue?page=$page&pageSize=$pageSize&status=$status');
    final res = await _http.get(uri, headers: {'Authorization': 'Bearer $accessToken'});
  ...
  Future<void> approve(String accessToken, String appealId) async {
    ...
    final res = await _http.post(uri, headers: {'Authorization': 'Bearer $accessToken'});
