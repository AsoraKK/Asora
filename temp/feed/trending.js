...
const logger = (0, azure_logger_1.getAzureLogger)('feed/trending');
...
const httpTrigger = async function (req, context) {
    ...
    try {
        ...
        const corsCheck = (0, http_utils_1.handleCorsAndMethod)(req.method, ['GET']);
        ...
        const validationResult = (0, validation_utils_1.validatePagination)(params.page, params.pageSize);
        if (!validationResult.valid) {
            return (0, http_utils_1.createErrorResponse)(400, validationResult.error || 'Validation failed');
        }
        ...
        return (0, http_utils_1.createErrorResponse)(500, 'Failed to load trending feed', process.env.NODE_ENV === 'development' ? errorMessage : undefined);
    }
};
...
function buildTrendingQuery(params) {
...
function buildTrendingCountQuery(params) {
    const parameters = [];
    const timeThreshold = getTimeThreshold(params.timeWindow || '24h');
    parameters.push({ name: '@timeThreshold', value: timeThreshold });
    const query = `
...
function getTimeThreshold(timeWindow) {
    const now = new Date();
    let hoursBack;
    switch (timeWindow) {
        case '1h':
...
function transformPostForResponse(post, _authHeader) {
...
function calculateTrendingStats(posts) {
    if (posts.length === 0) {
        return {
            averageEngagement: 0,
            topScore: 0,
