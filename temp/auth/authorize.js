...
/// 🏗️ Architecture: Azure Function implementing OAuth2 authorization endpoint
...
const logger = (0, azure_logger_1.getAzureLogger)('auth/authorize');
...
const httpTrigger = async function (req, context) {
    ...
    try {
        ...
        if (!userExists) {
            return createAuthError(authRequest.redirect_uri, authRequest.state, 'access_denied', 'User not found or not authorized');
        }
        ...
        if (queryParams.redirect_uri && queryParams.state) {
            return createAuthError(queryParams.redirect_uri, queryParams.state, 'server_error', 'Internal server error');
        }
        return (0, http_utils_1.createErrorResponse)(500, 'Authorization failed', process.env.NODE_ENV === 'development' ? errorMessage : undefined);
    }
};
function parseAuthorizeRequest(params) {
    return {
...
function validateAuthorizeRequest(request) {
    ...
    // Validate client_id
    const clientIdValidation = (0, validation_utils_1.validateText)(request.client_id, 1, 100);
    ...
    const stateValidation = (0, validation_utils_1.validateText)(request.state, 1, 255);
...
function createAuthError(redirectUri, state, error, errorDescription) {
    try {
        ...
        return (0, http_utils_1.createErrorResponse)(400, error, errorDescription);
    }
}
// Register the function with Azure Functions runtime
functions_1.app.http('auth-authorize', {
    methods: ['GET', 'OPTIONS'],
    authLevel: 'anonymous',
