...
const exportRateLimiter = (0, rate_limiter_1.createRateLimiter)({
    ...
    maxRequests: 1,
    keyGenerator: (0, rate_limiter_1.endpointKeyGenerator)('privacy_export')
});
async function exportUser(request, context) {
    ...
    try {
        ...
        if ((0, auth_utils_1.isHttpError)(err)) {
            return (0, auth_utils_1.json)(err.status, err.body);
        }
        ...
        return {
            ...
            body: JSON.stringify({ error: 'INTERNAL_ERROR', detail: err?.message, exportId }),
        };
    }
}
