...
exports.voteOnAppeal = voteOnAppeal;
const zod_1 = require("zod");
const cosmos_1 = require("@azure/cosmos");
const auth_utils_1 = require("../shared/auth-utils");
// Request validation schema
const VoteOnAppealSchema = zod_1.z.object({
    appealId: zod_1.z.string().min(1),
    vote: zod_1.z.enum(['approve', 'reject']),
    reason: zod_1.z.string().min(10).max(500),
    confidence: zod_1.z.number().min(1).max(10).default(5),
    notes: zod_1.z.string().max(1000).optional()
});
async function voteOnAppeal(request, context) {
    ...
    try {
        ...
        try {
            ...
            if ((0, auth_utils_1.hasRole)(jwtPayload, 'admin')) {
                voterWeight = 3;
            }
            else if ((0, auth_utils_1.hasRole)(jwtPayload, 'moderator')) {
                voterWeight = 2;
            }
            else {
                voterWeight = 1;
        ...
        if (hasQuorum) {
            ...
            await updateContentBasedOnDecision(database, appealDoc.contentId, appealDoc.contentType, finalDecision, context);
        }
        else {
 ...
 */
async function updateContentBasedOnDecision(database, contentId, contentType, decision, context) {
    try {
