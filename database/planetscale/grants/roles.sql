...
GRANT SELECT, DELETE ON trust.accountability_signals, trust.reputation_balances, system.idempotency_keys TO lythaus_jobs;
...
GRANT SELECT, DELETE ON identity.provider_links, identity.email_credentials, identity.contact_emails, identity.handles, identity.email_verification_tokens, identity.password_reset_tokens, identity.user_region_preferences, identity.admin_memberships TO lythaus_privacy;
GRANT SELECT, DELETE ON social.profiles, social.profile_private_fields, social.custom_feeds TO lythaus_privacy;
GRANT SELECT, DELETE ON editorial.applications, editorial.memberships TO lythaus_privacy;
