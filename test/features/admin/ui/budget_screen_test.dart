  ...
  Future<BudgetUpdateResult> updateBudget(double amount) {
    ...
    return Future.value(
      _updateBuilder?.call(amount) ??
          BudgetUpdateResult(budget: _budgetBuilder(), azureSynced: true),
    );
  }
}

BudgetInfo _budgetInfo({double amount = 250}) {
  return BudgetInfo(
    amount: amount,
...
void main() {
  ...
  testWidgets(
    'loading, error, loaded, dirty, saving, and saved states render',
    (tester) async {
      ...
      notifier.state = notifier.state.copyWith(
        status: BudgetScreenStatus.saving,
      );
      await tester.pump();
