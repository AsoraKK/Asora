...
import { getCacheConfig, isRedisCacheEnabled, isEdgeCacheEnabled, shouldCollectTelemetry } from './cacheConfig';
...
describe('Cache Configuration', () => {
    ...
    describe('getCacheConfig', () => {
        ...
        it('should throw when FEED_CACHE_BACKEND is invalid', () => {
            process.env.FEED_CACHE_BACKEND = 'invalid';
            expect(() => getCacheConfig()).toThrow('Invalid FEED_CACHE_BACKEND: invalid. Must be one of: edge, redis, none');
        });
    });
    ...
    describe('shouldCollectTelemetry', () => {
        ...
        it('should prioritize query param when both are present but header is wrong', () => {
            process.env.EDGE_TELEMETRY_SECRET = 'test-secret';
            const request = mockRequest(
                { 'x-debug-telemetry': 'wrong-secret' },
                { telemetry: '1' }
            );
            expect(shouldCollectTelemetry(request)).toBe(true);
        });
