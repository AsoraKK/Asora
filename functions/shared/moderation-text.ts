...
export async function moderateProfileText(text: string, userId?: string): Promise<TextModerationOutcome> {
  ...
  const attempt = async (provider: 'hive' | 'acs'): Promise<TextModerationOutcome> => {
    ...
    if (provider === 'hive') {
      ...
      let decision: ModerationDecision = parsed.action === 'reject' ? 'reject' : parsed.action === 'review' ? 'review' : 'allow';
      if (score >= thr.reject) decision = 'reject'; else if (score >= thr.review) decision = 'review';
      ...
      const decision: ModerationDecision = res.score >= thr.reject ? 'reject' : res.score >= thr.review ? 'review' : 'allow';
      return { provider: 'acs', decision, score: res.score, details: res.categoryScores, durationMs: Date.now() - start };
    }
  };
  ...
  for (let i = 0; i <= cfg.retries; i++) {
    try {
      ...
      logger.info('Text moderation decision', { provider: out.provider, decision: out.decision, score: out.score, durationMs: out.durationMs });
  ...
  if (cfg.enableFallback) {
    try {
      ...
      logger.info('Text moderation fallback decision', { provider: out.provider, decision: out.decision, score: out.score, durationMs: out.durationMs });
      logPerformanceMetric(logger, 'moderation_text_latency_ms', out.durationMs, 'ms');
      return out;
    } catch (e2) {
      lastErr = e2;
