...
export async function postCreate(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        ...
        try {
            ...
            context.log(`Hive AI analysis: score=${hiveResult.score}, decision=${hiveResult.decision}, categories=${Object.keys(hiveResult.categories).join(',')}`);
        ...
        return {
            ...
            jsonBody: { 
                ...
                message: process.env['NODE_ENV'] === 'development' ? error.message : 'Unable to create post'
            }
        };
    }
}
