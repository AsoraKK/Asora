    ...
    if (authorIds.length) {
      try {
        const { resources } = await this.users()
          .items.query({
            query:
              'SELECT u.id, u.reputationLevel, u.consistency FROM u WHERE ARRAY_CONTAINS(@ids, u.id)',
            parameters: [{ name: '@ids', value: authorIds }],
          })
          .fetchAll();

        for (const u of resources as Array<{
          id: string;
          ...
          signalsMap.set(u.id, {
            authorId: u.id,
            reputationLevel: Math.min(
              5,
              Math.max(1, u.reputationLevel ?? 1)
            ) as AuthorSignals['reputationLevel'],
