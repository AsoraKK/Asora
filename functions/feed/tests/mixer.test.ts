...
describe("Mixer", () => {
  ...
  it("respects different local/global ratios", () => {
    const halfItems = [
      ...Array.from({ length: 6 }, (_, idx) => makeItem(`lh${idx}`, "US", 1 - idx * 0.01, new Date().toISOString())),
      ...Array.from({ length: 6 }, (_, idx) => makeItem(`gh${idx}`, "CA", 0.9 - idx * 0.01, new Date().toISOString())),
    ];
    const halfResult = mixer.apply(halfItems, makeContext({ localToGlobalRatio: 0.5, pageSize: 12 }));
    expect(halfResult.filter((i) => i.region === "US")).toHaveLength(6);

    const ratioItems = [
      ...Array.from({ length: 2 }, (_, idx) => makeItem(`l${idx}`, "US", 1 - idx * 0.01, new Date().toISOString())),
      ...Array.from({ length: 8 }, (_, idx) => makeItem(`g${idx}`, "CA", 0.9 - idx * 0.01, new Date().toISOString())),
    ];
    const ratioResult = mixer.apply(ratioItems, makeContext({ localToGlobalRatio: 0.2, pageSize: 10 }));
  ...
  it("sorts chronologically versus balanced", () => {
    ...
    const chronological = mixer.apply(items, makeContext({ userPrefs: { rankMode: "chronological" }, region: undefined }));
    expect(chronological.map((i) => i.id)).toEqual(["new", "mid", "old"]);

    const balanced = mixer.apply(items, makeContext({ userPrefs: { rankMode: "balanced" }, region: undefined }));
