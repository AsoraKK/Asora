...
describe('Filter', () => {
  it('drops candidates below AI threshold', () => {
    ...
    const result = filter.apply(
      [
        makeCandidate({ id: 'ok', aiHumanScore: 0.9 }),
        makeCandidate({ id: 'bot', aiHumanScore: 0.05 }),
      ],
      ctx,
      following
    );
  ...
  it('treats missing keywords as non-matching when includes are required', () => {
    ...
    const noKeywords = filter.apply(
      [makeCandidate({ keywords: undefined })],
      includeCtx,
      new Set()
    );
    ...
    const missingKeywords = filter.apply(
      [makeCandidate({ keywords: undefined })],
      excludeCtx,
      new Set()
    );
