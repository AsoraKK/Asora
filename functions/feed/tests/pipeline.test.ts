...
describe("pipeline integration", () => {
	...
	it("uses personalized candidate sources when mode is personalized", async () => {
		const followingCands = [
			makeCandidate({ id: "follow-1" }),
			makeCandidate({ id: "follow-2" }),
		];
		...
		expect(adapterStub.listTrendingPosts).toHaveBeenCalledWith({ limit: 25, regions: ["US", "CA"] });
