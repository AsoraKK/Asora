...
describe('FairnessPolicy', () => {
  ...
  it.each([{ pageSize: 10 }, { pageSize: 20 }, { pageSize: 40 }])(
    'respects scaled floors and caps for pageSize $pageSize',
    ({ pageSize }) => {
      const pool = [
        ...cohortItems(5, 20, 'hi'),
        ...cohortItems(4, 20, 'c4'),
  ...
  it('halts exploration loop when all remaining candidates violate caps', () => {
    ...
    const pool = [...sameAuthorItems(3, 6, 'solo'), ...cohortItems(2, 2, 'other')];
  ...
  it('treats missing floor configuration as zero floor', () => {
    ...
    const pool = [...cohortItems(4, 4, 'gap'), ...cohortItems(3, 4, 'c3')];
  ...
  it('skips over authors once the per-author cap is reached in floors', () => {
    ...
    const pool = [...sameAuthorItems(3, 4, 'repeat'), ...cohortItems(3, 4, 'others')];
  ...
  it('skips final fill items once cohort hits cap', () => {
    ...
    const pool = [...cohortItems(3, 6, 'c3'), ...cohortItems(2, 3, 'c2')];
  ...
  it('avoids re-adding already chosen items during exploration', () => {
    ...
    const duplicate = makeOutputItem({
      id: 'dup',
      authorId: 'dup',
      cohort: 3,
      _score: 5,
