...
describe('exportService - user verification', () => {
  it('handles missing user gracefully with minimal profile', async () => {
    // User read returns nothing - service creates minimal profile
    mockRead.mockResolvedValueOnce({ resource: null });

    // Empty data for all containers
    mockQuery
    ...
    expect(body.userProfile.id).toBe('missing-user');
  });
});
...
describe('exportService - successful export', () => {
  ...
  it('handles Cosmos user read errors gracefully', async () => {
    // User read fails - service handles it gracefully
    mockRead.mockRejectedValueOnce(new Error('Cosmos connection failed'));

    // Provide empty data for queries (they may still be called)
    mockQuery
    ...
    expect(response.status).toBe(200);
    const body = JSON.parse(response.body || '{}');
    expect(body.userProfile.id).toBe('user-1');
