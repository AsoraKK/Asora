...
describe('Daily Post Limit Service', () => {
  ...
  describe('DailyPostLimitExceededError', () => {
    ...
    it('should produce correct response object', () => {
      ...
      expect(response.current).toBe(10);
  ...
  describe('Tier-based rate limiting scenarios', () => {
    it('free user hits limit at 10 posts', async () => {
      const userId = 'free-scenario-user';
      const date = getUtcDateString();

      // Post 10 times (should all succeed)
      for (let i = 0; i < 10; i++) {
    ...
    it('premium user does not hit limit at 10 posts', async () => {
    ...
    it('premium user hits limit at 100 posts', async () => {
      ...
      // 100th post should succeed
      const result = await checkAndIncrementPostCount(userId, 'premium');
      expect(result.success).toBe(true);
      expect(result.newCount).toBe(100);

      // 101st post should fail
