...
export async function getFlaggedContent(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        ...
        if (userContext.role !== 'admin' && userContext.role !== 'moderator') {
            context.warn(`Unauthorized dashboard access attempt by user ${userContext.userId} with role ${userContext.role}`);
            return {
                status: 403,
                jsonBody: { 
                ...
                try {
                    ...
                    const flaggedItem: FlaggedContentItem = {
                        ...
                        escalation: {
                            ...
                            daysSinceEscalation,
                            actionsTaken: contentFlags
                                .filter(f => f.actionTaken)
                                .map(f => f.actionTaken.type)
                        }
                    };

                    flaggedItems.push(flaggedItem);
                } catch (flagError: any) {
                    context.warn(`Error processing flags for ${contentType} ${content.id}: ${flagError.message}`);
                    continue;
                }
            }
        }
        ...
        flaggedItems.sort((a, b) => {
            switch (params.sortBy) {
                case 'oldest':
                    return new Date(a.escalation.escalatedAt).getTime() - new Date(b.escalation.escalatedAt).getTime();
                case 'mostFlags':
                ...
                case 'newest':
                default:
                    return new Date(b.escalation.escalatedAt).getTime() - new Date(a.escalation.escalatedAt).getTime();
            }
        });
        ...
        const summary = {
            ...
            avgResponseTime: flaggedItems.length > 0 
                ? Math.round(flaggedItems.reduce((sum, item) => sum + item.escalation.daysSinceEscalation, 0) / flaggedItems.length * 10) / 10
                : 0,
            oldestPending: flaggedItems.length > 0 
        ...
        context.log(`✅ Admin dashboard accessed by ${userContext.email}: ${paginatedItems.length} items returned`);
        ...
        return {
            ...
            jsonBody: {
                ...
                message: process.env['NODE_ENV'] === 'development' ? error.message : 'Unable to load flagged content'
            }
        };
    }
}
