...
export async function moderateContent(
  ...
  try {
    ...
    if (userContext.role !== 'admin' && userContext.role !== 'moderator') {
      context.warn(
        `Unauthorized moderation attempt by user ${userContext.userId} with role ${userContext.role}`
      );
      return {
        status: 403,
        jsonBody: {
    ...
    const collectionName =
      moderationRequest.contentType === 'user' ? 'users' : `${moderationRequest.contentType}s`;
    const contentContainer = getContainer(collectionName);

    let targetContent;
    try {
      const { resource } = await contentContainer
        .item(moderationRequest.contentId, moderationRequest.contentId)
        .read();
    ...
    try {
      await contentContainer
        .item(moderationRequest.contentId, moderationRequest.contentId)
        .patch(contentUpdates);
      contentUpdated = true;
      context.log(
        `✅ Content ${moderationRequest.contentId} updated with decision: ${moderationRequest.decision}`
      );
    ...
    if (moderationRequest.notifyUsers) {
      ...
      for (const notification of notificationTargets) {
        console.log(
          `📧 Notification queued: ${notification.type} to ${notification.recipientId} - ${notification.message}`
        );
        notificationsSent++;
      }

      context.log(
        `📧 ${notificationsSent} notifications queued for decision: ${moderationRequest.decision}`
      );
    }
      ...
      jsonBody: {
        ...
        message:
          process.env['NODE_ENV'] === 'development'
            ? error.message
            : 'Unable to process moderation decision',
      },
    };
  }
}
