...
export async function voteOnAppeal(
  ...
  try {
    ...
    // 4. Rate limiting - Check user's votes in the last hour (rolling window)
    ...
    const { resources: recentVotes } = await votesContainer.items.query(rateLimitQuery).fetchAll();
    if (recentVotes.length >= VOTING_CONFIG.MAX_VOTES_PER_HOUR) {
      const resetTime = new Date(now.getTime() + 3600 * 1000).toISOString(); // Reset in 1 hour from now
      return {
        status: 429,
        jsonBody: {
    ...
    let appeal;
    try {
      const { resource } = await appealsContainer
        .item(voteRequest.appealId, voteRequest.appealId)
        .read();
      appeal = resource;
    ...
    if (quorumMet || timeoutReached) {
      try {
        ...
        context.log(
          `🗳️ TALLY TRIGGER: Appeal ${voteRequest.appealId} reached quorum (${totalVotes} votes) or timeout`
        );
        tallyTriggered = true;
        ...
        context.warn(
          `Failed to trigger tally for appeal ${voteRequest.appealId}: ${tallyError.message}`
        );
      }
    }
    ...
    context.log(
      `✅ Vote cast by ${userContext.email}: ${voteRequest.vote} on appeal ${voteRequest.appealId} (${totalVotes} total votes)`
    );
    ...
    const result: VoteResult = {
      ...
      rateLimitInfo: {
        votesInLastHour: recentVotes.length + 1,
        maxPerHour: VOTING_CONFIG.MAX_VOTES_PER_HOUR,
    ...
    return {
      ...
      jsonBody: {
        ...
        message:
          process.env['NODE_ENV'] === 'development' ? error.message : 'Unable to process vote',
      },
    };
  }
}
