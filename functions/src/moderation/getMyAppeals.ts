...
export async function getMyAppeals(
  ...
  try {
    ...
    for (const appeal of userAppeals) {
      ...
      try {
        if (appeal.contentType !== 'user') {
          ...
          const contentItem = await contentContainer
            .item(appeal.contentId, appeal.contentId)
            .read();
          contentTitle =
            contentItem.resource?.title || contentItem.resource?.content?.substring(0, 50) + '...';
        }
      } catch {
    ...
    const { resources: allAppeals } = await appealsContainer.items
      .query(allUserAppealsQuery)
      .fetchAll();
