...
function buildInactiveResponse(): HttpResponseInit {
  return {
    ...
    body: JSON.stringify({
      ...
      message: 'This account is disabled and cannot perform this action.',
    }),
  };
}
  ...
  return requireAuth(async (req, context) => {
    ...
    try {
      ...
      if (!resource || resource.isActive === false) {
        context.log('auth.requireActiveUser.disabled', { userId });
        return buildInactiveResponse();
      }
    } catch (error) {
      context.log('auth.requireActiveUser.error', {
        userId,
