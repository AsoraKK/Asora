 ...
 */
export function checkQuorum(votes: VoteRecord[], appealCreatedAt: string): { 
    quorumMet: boolean; 
    timeoutReached: boolean; 
    reason: string 
} {
 ...
 */
export function calculateOutcome(votes: VoteRecord[], appealCreatedAt: string): AppealVotingSummary['outcome'] {
...
export function generateVotingSummary(
    ...
    const expiresAt = new Date(
        new Date(appealCreatedAt).getTime() + 
        VOTING_CONFIG.APPEAL_EXPIRY_DAYS * 24 * 60 * 60 * 1000
    ).toISOString();
...
export function calculateVotingUrgency(
    ...
    const minutesSinceCreated = (Date.now() - new Date(appealCreatedAt).getTime()) / (1000 * 60);
    const hoursUntilExpiry = (new Date(getAppealExpiration(appealCreatedAt)).getTime() - Date.now()) / (1000 * 60 * 60);
    
    let urgencyScore = 0;
    
    // Time-based urgency (more urgent as timeout approaches)
    urgencyScore += Math.max(0, (VOTING_CONFIG.TIMEOUT_MINUTES - minutesSinceCreated) / VOTING_CONFIG.TIMEOUT_MINUTES) * 50;
