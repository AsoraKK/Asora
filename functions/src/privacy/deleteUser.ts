...
export async function deleteUser(
  ...
  try {
    ...
    if (existingUser?.accountDeleted) {
      ...
      _logTelemetryEvent(
        context,
        'privacy_delete_already_deleted',
        userId,
        0,
        Date.now() - startTime
      );
    ...
    context.info(
      `Content marked as deleted - Posts: ${contentChanges.postsUpdated}, Comments: ${contentChanges.commentsUpdated}`
    );
 ...
 */
async function _removeAncillaryDocuments(
  userId: string,
  context: InvocationContext
): Promise<number> {
  let removedCount = 0;
  ...
  for (const containerName of containerNames) {
    try {
      ...
      if (documents.length > 0) {
        context.info(
          `Removed ${documents.length} documents from ${containerName} for user: ${userId}`
        );
      }
    } catch (error) {
