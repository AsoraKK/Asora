...
async function handler(req: Authed, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    const request: DsrRequest = {
      ...
      attempt: 0,
      review: {},
      audit: [createAuditEntry({ by: requestedBy, event: 'delete.enqueued', meta: note ? { note } : undefined })],
    };
    ...
    context.log('dsr.delete.enqueued', { ...event, queue });
    trackDsrEvent('dsr.delete.enqueued', event);
