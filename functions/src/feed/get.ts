...
async function queryPostsFromCosmos(
  userId: string,
  query: FeedQuery,
  context: InvocationContext
): Promise<PostForRanking[]> {
...
async function queryFeedFromCosmos(
  userId: string,
  query: FeedQuery,
  userTier: string,
  context: InvocationContext
  ...
  if (isAuthenticated) {
    ...
    if (isEdgeCacheEnabled()) {
      headers['Cache-Control'] =
        `public, s-maxage=${cacheConfig.ttlSeconds}, stale-while-revalidate=${cacheConfig.ttlSeconds * 2}`;
...
export async function getFeedInternal(
  request: HttpRequest,
  context: InvocationContext
        ...
        algorithm: {
          ...
          ranking: collectTelemetry
            ? {
                ...
                avgScore:
                  rankedPosts.reduce((sum, p) => sum + p.score, 0) / rankedPosts.length || 0,
                cacheHit: false,
              }
            : undefined,
        },
