...
} from '../../shared/cacheConfig';

describe('Feed Cache Headers', () => {
  const mockGetCacheConfig = getCacheConfig as jest.MockedFunction<typeof getCacheConfig>;
  const mockIsEdgeCacheEnabled = isEdgeCacheEnabled as jest.MockedFunction<
    typeof isEdgeCacheEnabled
  >;
  const mockIsRedisCacheEnabled = isRedisCacheEnabled as jest.MockedFunction<
    typeof isRedisCacheEnabled
  >;
  const mockShouldCollectTelemetry = shouldCollectTelemetry as jest.MockedFunction<
    typeof shouldCollectTelemetry
  >;
