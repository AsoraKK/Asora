  ...
  handler: async (_t: Timer, ctx: InvocationContext) => {
    ...
    for (const u of remindUsers) {
      const { resources: userPosts } = await posts.items
        .query({
          query: 'SELECT TOP 1 p.id FROM p WHERE p.authorId=@uid',
          parameters: [{ name: '@uid', value: u.id }],
        })
        .fetchAll();
      if (userPosts.length === 0 && u.email)
        await sendEmail(u.email, 'Create your first post to unlock full access');
    }
