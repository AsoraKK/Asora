 ...
 */
export async function privacyCleanupTimer(
  myTimer: Timer,
 ...
 */
async function _cleanupDeletedPosts(
  cutoffDate: Date,
  context: InvocationContext
): Promise<{ postsRemoved: number }> {
  ...
  try {
    ...
    const { resources: deletedPosts } = await container.items
      .query({
        query: `
 ...
 */
async function _cleanupDeletedComments(
  cutoffDate: Date,
  context: InvocationContext
): Promise<{ commentsRemoved: number }> {
  ...
  try {
    ...
    const { resources: deletedComments } = await container.items
      .query({
        query: `
 ...
 */
async function _anonymizeOldUserData(
  context: InvocationContext
 ...
 */
async function _cleanupExpiredExportRequests(
  context: InvocationContext
): Promise<{ exportsCleared: number }> {
