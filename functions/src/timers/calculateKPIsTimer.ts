...
async function calculateWAU(context: InvocationContext): Promise<number> {
  ...
  try {
    ...
    const querySpec = {
      ...
      `,
      parameters: [{ name: '@weekAgo', value: weekAgo.toISOString() }],
    };
...
async function calculateDay1Retention(context: InvocationContext): Promise<number> {
  ...
  try {
    ...
    const { resources: newUsers } = await usersContainer.items
      .query<number>(newUsersQuery)
      .fetchAll();
    ...
    const { resources: retainedUsers } = await usersContainer.items
      .query<number>(retainedUsersQuery)
      .fetchAll();
    ...
    context.info(
      `📊 Day 1 retention: ${(retentionRate * 100).toFixed(2)}% (${retainedUserCount}/${newUserCount})`
    );
...
async function calculateDay7Retention(context: InvocationContext): Promise<number> {
  ...
  try {
    ...
    const { resources: cohortUsers } = await usersContainer.items
      .query<number>(cohortQuery)
      .fetchAll();
    ...
    context.info(
      `📊 Day 7 retention: ${(retentionRate * 100).toFixed(2)}% (${retainedCount}/${cohortSize})`
    );
...
async function calculateAppealMetrics(context: InvocationContext): Promise<ModerationMetrics> {
  ...
  try {
    ...
    const uphelAppeals = appeals.filter(
      appeal =>
    ...
    context.info(
      `📊 Appeal metrics - SLA: ${avgAppealSlaHours.toFixed(2)}h, FPR: ${(falsePositiveRate * 100).toFixed(2)}%`
    );
...
async function calculateKPIsInternal(myTimer: Timer, context: InvocationContext): Promise<void> {
  ...
  try {
    ...
    context.info(
      `   • False Positive Rate: ${(moderationMetrics.falsePositiveRate * 100).toFixed(2)}%`
    );
