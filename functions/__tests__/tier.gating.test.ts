...
jest.mock('../shared/hive-client', () => ({
  ...
  HiveAIClient: {
    parseModerationResult: () => ({ action: 'accept', confidence: 0.1, flaggedCategories: [] }),
  },
}));
    ...
    database: jest.fn().mockReturnValue({
      container: jest.fn().mockImplementation((name: string) => {
          ...
          return {
            item: jest.fn().mockReturnValue({
              read: jest.fn().mockResolvedValue({
                resource: { id: 'user-1', tier: 'free', reputationScore: 0 },
              }),
            }),
          };
        }
        ...
        if (name === 'reputation_audit') {
          return {
            item: jest.fn().mockReturnValue({ read: jest.fn().mockRejectedValue({ code: 404 }) }),
...
function ctx(): InvocationContext {
  return { invocationId: 't', log: jest.fn() } as any;
}
