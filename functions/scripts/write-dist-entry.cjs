...
// 4) SAFE entrypoint for Flex: load ONLY classic health (zero deps, cannot fail)
//    All other routes are registered via their individual function.json files
//    This prevents host startup failure if any dependencies are missing
fs.writeFileSync(destEntry,
`// Auto-generated by scripts/write-dist-entry.cjs
// Safe Flex entrypoint: load only classic health (zero dependencies)
// All other routes are registered via their individual function.json files
require('./health/index.js');
`
);
console.log('[write-dist-entry] Wrote safe health-only entrypoint to', destEntry);
