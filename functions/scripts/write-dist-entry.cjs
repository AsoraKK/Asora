...
if (!fs.existsSync(srcEntry)) {
  console.error('Missing', srcEntry);
  process.exit(1);
}

fs.mkdirSync(distDir, { recursive: true });

// classic /api/health (always)
...
// 3) Prune tests from dist
function prune(dir) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    const p = path.join(dir, e.name);
    if (e.isDirectory()) {
      if (e.name === '__tests__' || e.name === 'tests') {
    ...
    if (/\.(test|spec)\.[cm]?js$/.test(e.name) || /\.test\.d\.ts$/.test(e.name)) {
...
// 4) Hardened bootstrap that never imports health
...
fs.writeFileSync(
  destEntry,
  `try{require('./src/index.js');}catch(e){console.error('[bootstrap] ./src/index.js',e&&e.message||e);for(const m of ${JSON.stringify(
    fallbackModules
