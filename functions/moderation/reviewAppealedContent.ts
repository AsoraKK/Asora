...
import { isEligibleForVoting, calculateVotingUrgency, generateVotingSummary, VOTING_CONFIG } from '../shared/moderationUtils';
import Joi from 'joi';
...
export async function reviewAppealedContent(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        ...
        // Query appeals with community review queue
        const appealsQuery = {
            query: `
                SELECT * FROM c 
        ...
        const { resources: activeAppeals } = await appealsContainer.items.query(appealsQuery).fetchAll();
        ...
        for (const appeal of activeAppeals) {
            try {
                ...
                let content;
                try {
                    const { resource } = await contentContainer.item(appeal.contentId, appeal.contentId).read();
                    content = resource;
                ...
                // Calculate urgency
                const urgencyScore = calculateVotingUrgency(appealVotes, appeal.createdAt, content.flagCount || 1);
                ...
                if (appealVotes.length >= VOTING_CONFIG.MINIMUM_VOTES - 1) urgencyFactors.push('Close to quorum');
        ...
        return {
            ...
            jsonBody: {
                ...
                message: process.env['NODE_ENV'] === 'development' ? error.message : 'Unable to load appeals'
            }
        };
    }
}
