...
interface AppealReview {
  ...
  submittedAt: string;
  expiresAt: string;
  timeRemaining: number; // Hours until expiry
  originalContent: {
    text?: string;
    imageUrls?: string[];
...
export async function reviewAppealedContent(
  ...
  try {
    ...
    const enrichedAppeals: AppealReview[] = [];
    ...
    for (const appeal of appeals) {
      try {
        ...
        try {
          ...
          if (content) {
            originalContent = {
              ...
              imageUrls: (content as any).imageUrls || (content as any).images || [],
              metadata: {
        ...
        try {
          ...
          if (flags.length > 0) {
            const reasons = flags.map((f: any) => f.reason).filter(Boolean);
            reportContext = {
        ...
        const enrichedAppeal: AppealReview = {
          appealId: appeal.id,
