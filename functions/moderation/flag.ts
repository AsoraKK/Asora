...
export async function flagContent(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        ...
        const duplicateQuery = {
            query: 'SELECT * FROM c WHERE c.userId = @userId AND c.targetType = @targetType AND c.targetId = @targetId',
            parameters: [
                { name: '@userId', value: userContext.userId },
        ...
        const { resources: existingFlags } = await flagsContainer.items.query(duplicateQuery).fetchAll();
        ...
        const targetContainer = getContainer(value.targetType === 'user' ? 'users' : `${value.targetType}s`);
        let targetContent;
        try {
        ...
        try {
            ...
            context.log(`Flag analysis: target=${value.targetType}:${value.targetId}, score=${hiveResult.score}, decision=${hiveResult.decision}`);
        } catch (hiveError: any) {
        ...
        const shouldAutoHide = hiveResult.score >= moderationConfig.appeal.autoHide || hiveResult.decision === 'block';
        ...
        if (shouldAutoHide) {
            ...
            try {
                ...
                context.log(`✅ Auto-hidden content: ${value.targetType}:${value.targetId} due to high AI confidence`);
        ...
        const { resources: allFlagsForContent } = await flagsContainer.items.query(escalationQuery).fetchAll();
        ...
        if (flagCount >= 3) {
            ...
            try {
                ...
                context.log(`🚨 ESCALATED: ${value.targetType}:${value.targetId} has ${flagCount} flags - marked for admin review`);
        ...
        context.log(`✅ Flag created: ${flagId} by user ${userContext.userId} for ${value.targetType}:${value.targetId}`);
        ...
        return {
            ...
            jsonBody: {
                ...
                message: process.env['NODE_ENV'] === 'development' ? error.message : 'Unable to process flag'
            }
        };
    }
}
