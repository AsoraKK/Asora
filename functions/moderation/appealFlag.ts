...
export async function appealFlag(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        ...
        const { resources: todayAppeals } = await appealsContainer.items.query(rateLimitQuery).fetchAll();
        ...
        const collectionName = appealRequest.contentType === 'user' ? 'users' : `${appealRequest.contentType}s`;
        const contentContainer = getContainer(collectionName);
        
        let targetContent;
        try {
            const { resource } = await contentContainer.item(appealRequest.contentId, appealRequest.contentId).read();
        ...
        const duplicateQuery = {
            query: 'SELECT * FROM c WHERE c.userId = @userId AND c.contentId = @contentId AND c.contentType = @contentType',
            parameters: [
                { name: '@userId', value: userContext.userId },
        ...
        const { resources: existingAppeals } = await appealsContainer.items.query(duplicateQuery).fetchAll();
        ...
        try {
            ...
            await contentContainer.item(appealRequest.contentId, appealRequest.contentId).patch(contentUpdate);
        ...
        const auditRecord = {
            ...
            metadata: {
                ...
                appealsToday: todayAppeals.length + 1,
                daysSinceModerationDecision: Math.floor((Date.now() - moderatedAt.getTime()) / (1000 * 60 * 60 * 24))
            }
        };
        ...
        return {
            ...
            jsonBody: {
                ...
                message: process.env['NODE_ENV'] === 'development' ? error.message : 'Unable to submit appeal'
            }
        };
    }
}
